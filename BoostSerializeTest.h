...
//
	...
	class bus_route
	{
		...
		using bus_stop_pointer = std::shared_ptr<bus_stop>;
	...
	public:
		bus_route() {}
		void append(bus_stop_pointer _bs)
		{
	...
	{
		using bus_stop_pointer = std::shared_ptr<bus_stop>;
		std::list<bus_stop_pointer>::const_iterator it;
	...
	int execute(int argc, char* argv[])
	{
		...
		// fill in the data
		// make a few stops
		bus_stop_pointer bs0 = std::make_shared<bus_stop_corner>(bus_stop_corner(
			gps_position(34, 135, 52.560f),
			gps_position(134, 22, 78.30f),
			"24th Street", "10th Avenue"
		));
		bus_stop_pointer bs1 = std::make_shared<bus_stop_corner>(bus_stop_corner(
			gps_position(35, 137, 23.456f),
			gps_position(133, 35, 54.12f),
			"State street", "Cathedral Vista Lane"
		));
		bus_stop_pointer bs2 = std::make_shared<bus_stop_destination>(bus_stop_destination(
			gps_position(35, 136, 15.456f),
			gps_position(133, 32, 15.300f),
			"White House"
		));
		bus_stop_pointer bs3 = std::make_shared<bus_stop_destination>(bus_stop_destination(
...
namespace MyXML
{
	...
	int execute(int argc, char* argv[])
	{
		...
		// save the schedule
		MyXML::save_schedule(original_schedule, filename.c_str());

		// ... some time later
		// make  a new schedule
		bus_schedule new_schedule;

		MyXML::restore_schedule(new_schedule, filename.c_str());
