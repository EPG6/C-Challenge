...
namespace string_library
{
	...
	template <typename CharT>
	using tstringview = std::basic_string_view<CharT>;
	// --------
	...
	template<typename CharT>
	inline tstring<CharT> replace(tstring<CharT> text, CharT const ch, CharT const subs)
	{
		while (true)
		{
			auto letter{ std::find(text.begin(),text.end(),ch) };
			if (letter == text.end())
				break;
			*letter = subs;
		}
	...
	template<typename CharT>
	inline std::vector<tstring<CharT>> split(tstring<CharT> text, const CharT delimiter)
	{
	...
	template<typename CharT>
	inline std::vector<tstring<CharT>> split(tstring<CharT> text, tstringview<CharT> delimiter)
	{
		auto delim{ delimiter[0] };
		for (auto el : delimiter)
		{
			if (el == delim)
				continue;
			text = replace(text, el, delim);
		}
		return split(text, delim);
	}

	template<typename CharT>
	inline tstring<CharT> Concatenate(std::vector<tstring<CharT>> words, const CharT delimiter)
	{
		tstring<CharT> text{};
		for (auto& word : words)
		{
			text += (word + tstring<CharT>{delimiter});
		}
		return text;
	}

	template<typename CharT>
	inline tstring<CharT> CapitaliseTitle(tstring<CharT> text)
	{
	...
	/*void execute()
	{
		...
		std::cout << "Shows a sample of the library's capabilities." << std::endl;
